...
public class MineSweeper {
    ...
    public void setupGUI(){
        ...
        UI.addButton("Automatic Solve", this::trySolve);
    ...
    public void trySolve() {
        int[][] state = getVisibleState();
        if (!anyExposed()) {
            // initially, pick a random spot to reveal
            int x = (int) Math.floor(Math.random()*state.length);
            int y = (int) Math.floor(Math.random()*state.length);
            tryExpose(x,y);
        }
    }
    ...
    private final Point[] transformations = new Point[]{
            new Point(-1, -1),
            new Point(-1, 0),
            new Point(-1, 1),
    ...
    public void exposeSquareAt(int row, int col){
        ...
        if (row < 0 || col < 0 || row >= squares.length || col >= squares[row].length) return;
        ...
        if (target.getAdjacentMines() == 0) {
            ...
            for (Point t : transformations) {
                exposeSquareAt(row + t.x, col + t.y);
            }
        }
    }
    ...
    public void makeGrid(){
        ...
        for (int row=0; row < ROWS; row++){
            for (int col=0; col<COLS; col++){
                ...
                this.squares[row][col] = new Square(isMine, row, col);

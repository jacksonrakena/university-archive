        ...
        while (running){         // each time step, check whether the simulation should pause.
            ...
            for (Patient p : treatmentRoom) {
                if (p.completedCurrentTreatment()) {
                    ...
                    newTreatmentRoom.add(p);
                }
            }

            Queue<Patient> newWaitingRoom = (waitingRoom instanceof PriorityQueue) ? (new PriorityQueue<>()) : new ArrayDeque<>();
            for (Patient p : waitingRoom) {
                if (newTreatmentRoom.size() < MAX_PATIENTS) {
                    // INITIATE TREATMENT
                    newTreatmentRoom.add(p);
                } else {
                    // CONTINUE WAITING
                    newWaitingRoom.add(p);
                    p.waitForATick();
                }
            }

...
#define CLIENT_FORKED 100 // handle_connection mode representing that we are in a subprocess
#define FORK_FAILED 101 // handle_connection mode representing that the fork failed and we are still single-processed
 ...
 */
int handle_connection(int socket_id, int mode) {
    printf("accepted socket %d \n", socket_id);

    char* recv_buffer = calloc(BUFFER_SIZE, sizeof(char));
    ...
    // End the current process if we're a child (forked) process and we've reached the end of the connection.
    if (mode == CLIENT_FORKED) {
...
int main(int argc, char *argv[])
{
    ...
    while (server_listening) {
        ...
        if (fork_result == -1) {
            handle_connection(opened_socket, FORK_FAILED);
        } else if (fork_result == 0) {
            handle_connection(opened_socket, CLIENT_FORKED);
        } else {
            printf("successfully forked as process id %d \n", fork_result);
        }
    }

    printf("shutting down\n");
    if (shutdown(socket_descriptor, SHUT_RDWR) != 0) {
        error_exit("failed to close socket. further clean up may be necessary.\n");
    }
    return 0;
}
...
public class Board {
	...
	private Cell[] cells;
	/**
	 * The correct configuration of the board, before the cells were randomly manipulated.
	 */
	private Cell[] correctConfiguration;
	...
	public Board(int[] image, int width, int dimension) {
		...
		this.correctConfiguration = this.cells.clone();
	}
	...
	public boolean isFinished() {
		for (int i = 0; i < this.cells.length; i++) {
			// check that this cell is equal to the same cell in its position in the original layout
			if (this.cells[i] != this.correctConfiguration[i]) return false;

			// check that all cells are upright
			if (this.cells[i] != null && this.cells[i].getRotation() % 360 != 0)  return false;
		}
		return true;
	}
	...
	public void swap(Location l1, Location l2) {
		int id1 = (l1.row * this.dimension) + l1.col;
		int id2 = (l2.row * this.dimension) + l2.col;
		Cell temp = cells[id1];
		cells[id1] = cells[id2];
		cells[id2] = temp;
	}

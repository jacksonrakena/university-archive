...
public abstract class Tetromino {
  ...
  public boolean overlap(Board b) {
    ...
    // For all c in (0..3), read from the board (x(c), y(c)), and check that its empty or our color
    return !Board.rangeT().allMatch(c -> b.read(x(c), y(c)).map(d -> d == Color.EMPTY || d == color()).orElse(false));
  }
